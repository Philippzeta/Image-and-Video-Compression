...
class IntraCodec:
    ...
    def image2symbols(self, img: np.array):
        ...
        # 2. 应用DCT变换
        dct_coeffs = self.dct.forward(ycbcr)

        # 3. 量化
        quantized = self.quant.forward(dct_coeffs)

        # 4. ZigZag扫描
        zigzagged = self.zigzag.forward(quantized)

        # 5. 零游程编码
        symbols = self.zerorun.forward(zigzagged)
        # YOUR CODE ENDS HERE
        return symbols
    ...
    def symbols2image(self, symbols, original_shape):
        ...
        # YOUR CODE STARTS HERE
        # 1. 零游程解码
        zigzagged = self.zerorun.inverse(symbols, patch_shape)

        # 2. 反ZigZag扫描
        quantized = self.zigzag.inverse(zigzagged)

        # 3. 反量化
        dct_coeffs = self.quant.inverse(quantized)

        # 4. 反DCT变换
        ycbcr = self.dct.inverse(dct_coeffs)
    ...
    def intra_encode(self, img: np.array):
        ...
        # 2. 使用Huffman编码器进行编码
        bitstream = self.huffman.forward(symbols)
        # YOUR CODE ENDS HERE
        return bitstream
    ...
    def intra_decode(self, bitstream, original_shape):
        ...
        # YOUR CODE STARTS HERE
        # 1. 使用Huffman解码器解码
        symbols = self.huffman.inverse(bitstream)
